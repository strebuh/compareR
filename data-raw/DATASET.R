...
usa_data[, status := sample(c("cool", "lame"), nrow(usa_data), replace=T, prob=c(.4, .6))]
...
na_perc <- 0.05
usa_data.na <- cbind(usa_data[, ..categ], purrr::map_df(usa_data[,..numb], ~.x[sample(c(TRUE, NA), prob = c(1-na_perc, na_perc), size = length(.x), replace = TRUE)]))
data.table::setDT(usa_data.na)
usethis::use_data(usa_data.na, overwrite = TRUE)
